...
class LeaksPilot {
    ...
    async * iterateOverSearchPerCodeLine(input, options) {
        const { tokenizer = /(.+?)[\n]*$/gm, trimToMaxLineSize = false, maxLineSize = 512, maxLinePadding = 64, breakOnLineMatch = false } = options || {}
        ...
        for await (let match of eachMatch(tokenizer, input)) {
            ...
            for await (let submatch of this.iterateOverSearch(line, options)) {
                if (line.length > maxLineSize) {
                    line = line.substring(Math.max(0, submatch.index - maxLinePadding), submatch.index + submatch.find.length + maxLinePadding)
                }
