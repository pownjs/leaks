...
const compileCheck = (check) => {
    const { regex = '', filterRegex = '', locationRegex, locationFilterRegex, flags = 'i', filterFlags = 'i', locationFlags = 'i', locationFilterFlags = 'i', entropy, entropyScore, ...rest } = check

    const r = regex ? regExp(regex, [...new Set('g' + flags)].join('')) : undefined
    ...
    const lfr = locationFilterRegex ? regExp(locationFilterRegex, locationFilterFlags) : undefined
        ...
        scan: function*(input, options) {
            ...
            for (let match of eachMatch(r, input)) {
                ...
                if (validate(exact, find)) {
                    yield { check, index, exact, find, entropy: calculateEntropy(exact), location }
                }
            }
        }
    }
